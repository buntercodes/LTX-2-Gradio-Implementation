...
from api.engine import load_pipeline, get_pipeline_status
...
async def api_generate_video(request: VideoGenerationRequest):
    ...
    if not is_loaded:
        raise HTTPException(
            status_code=400,
            detail="Pipeline not loaded. Call /api/v1/system/load first.",
        )
    try:
        task_id = await task_manager.submit_task(request)
        return VideoGenerationResponse(task_id=task_id, message="Task successfully queued.")
    except Exception as e:
