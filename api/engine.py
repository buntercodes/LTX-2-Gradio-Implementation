...
# Same as app.py constants
OUTPUT_DIR = Path("outputs")
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

MAX_CPU_TASKS = int(os.environ.get("MAX_CPU_TASKS", 2))        # Guard for 6 vCPUs (Text encoding/IO)
...
def get_pipeline_status() -> Tuple[bool, str]:
    if _pipeline is not None:
        return True, "Pipeline is loaded and ready."
...
def load_pipeline(config_req: SystemLoadRequest) -> str:
    ...
    if _pipeline is not None and _pipeline_config == config:
        ...
        return "Reusing existing pipeline instance."
    ...
    # Quantization
    quant_policy = None
    if config_req.quantization == "fp8-cast":
        quant_policy = QuantizationPolicy.fp8_cast()
    elif config_req.quantization == "fp8-scaled-mm":
    ...
    logger.info("Blackwell Optimized Pipeline ready.")
    return "Blackwell Optimized Pipeline successfully loaded."
...
def auto_load_from_env() -> bool:
    ...
    # Check if files actually exist before attempting load
    if Path(checkpoint).exists() and Path(upsampler).exists() and Path(gemma).exists():
        try:
            ...
            req = SystemLoadRequest(
                ...
                gemma_path=gemma,
                quantization=os.environ.get("LTX23_QUANTIZATION", "fp8-cast")
            )
            load_pipeline(req)
            logger.info(f"Pipeline auto-loaded using paths: {checkpoint}, {upsampler}, {gemma}")
            return True
...
async def process_generation_task(task: TaskState) -> None:
    """Core function to execute generation securely within the task queue."""
    ...
    task_manager.update_progress(task.task_id, 0.0, "Initialising generation | Blackwell Optimized…")
    ...
    status_parts = [
        f"Resolution: {height}x{width}, Frames: {num_frames}, FPS: {request.frame_rate}",
        f"Est. Duration: {duration_sec:.1f}s",
        f"Seed: {seed}",
    ]
    if images:
        status_parts.append(f"Conditioning Image Strength: {request.image_strength}")

    task_manager.update_progress(task.task_id, 0.05, "Waiting for CPU/Gemma slot…")

    # Run blocking GPU generation in an async executor if CPU bounds apply,
    # or just yield control using async locks
    async with CPU_LOCK:
    ...
    task_manager.update_progress(task.task_id, 0.85, "Encoding Video…")
    ...
    status_parts.append(f"Generation completed in {elapsed:.1f}s")
    
    if torch.cuda.is_available():
        peak_vram = torch.cuda.max_memory_allocated() / (1024**3)
        status_parts.append(f"Peak VRAM used: {peak_vram:.2f} GB")

    success_message = " | ".join(status_parts) + " | Blackwell Optimized"
