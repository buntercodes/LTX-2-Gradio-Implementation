...
class TaskManager:
    ...
    async def submit_task(self, request: VideoGenerationRequest) -> str:
        ...
        logger.info(f"Task {state.task_id} queued.")
        ...
        while self._is_running:
            try:
                ...
                if state.is_cancelled:
                    logger.info(f"Task {task_id} skipped because it was canceled while in queue.")
                    self.queue.task_done()
                    continue
    ...
    def start(self, processor_callback: Callable[[TaskState], Awaitable[None]]):
        ...
        # Start a number of worker tasks roughly equivalent to the semaphore limit
        num_workers = self.semaphore._value
        for _ in range(num_workers):
            task = asyncio.create_task(self._worker_loop())
            self._workers.append(task)
        logger.info("TaskManager background workers started.")
